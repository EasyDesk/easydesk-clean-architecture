...
public class AuditingStepTests
{
    ...
    {
        _agentProvider = Substitute.For<IAgentProvider>();
        _agentProvider.Agent.Returns(None);
    ...
    private Task<Result<Nothing>> Run<T>() where T : IReadWriteOperation, new() =>
        new AuditingStep<T, Nothing>(_auditStorage, _agentProvider, _auditConfigurer, _clock).Run(new T(), _next);
    ...
    {
        _agentProvider.Agent.Returns(agent);

...
public static partial class StaticImports
{
    ...
    public static Option<T> TryOption<T>(TryOperator<T> tryOperator) where T : notnull =>
        tryOperator(out var output) ? Some(output) : None;

    public static Option<T> TryOption<T1, T>(TryOperator<T1, T> tryOperator, T1 arg1) where T : notnull =>
        tryOperator(arg1, out var output) ? Some(output) : None;

    public static Option<T> TryOption<T1, T2, T>(TryOperator<T1, T2, T> tryOperator, T1 arg1, T2 arg2) where T : notnull =>
        tryOperator(arg1, arg2, out var output) ? Some(output) : None;

    public static Option<T> TryOption<T1, T2, T3, T>(TryOperator<T1, T2, T3, T> tryOperator, T1 arg1, T2 arg2, T3 arg3) where T : notnull =>
        tryOperator(arg1, arg2, arg3, out var output) ? Some(output) : None;

    public static Option<T> TryOption<T1, T2, T3, T4, T>(TryOperator<T1, T2, T3, T4, T> tryOperator, T1 arg1, T2 arg2, T3 arg3, T4 arg4) where T : notnull =>
